...
class Comments(MixinComment):
    ...
    async def post(self):
        ...
        comment, level = await tree.insert_tree(Comment, self.request['conn'], parent_id, values)
        ...
        return json_response(values, status=201, socket_action=Notification.ACTION['Push comment'])
